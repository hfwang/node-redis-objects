...
module.exports = exports = function Hash(key, myRedis, options) {
...
var aliasMethod = require('./util').aliasMethod.bind(undefined, exports);
...
exports.prototype.bulkSet = function(values, callback /* = null */) {
  var args = [this.key];
  for (var k in values) {
    args.push(k, this.toRedis(values[k], self.options.marshalKeys[k]));
  }
...
exports.prototype.bulkGet = function(fields, callback) {
  this.redis().hmget(this.key, fields, function(e, values) {
    ...
    for (var i = 0; i < fields.length; i++) {
      hash[fields[i]] = this.fromRedis(values[i], self.options.marshalKeys[fields[i]]);
    }
    callback(e, hash);
  });
};
...
exports.prototype.incrby = function(field, val, callback) {
  ...
  this.redis().incrby(this.key, field, val, callback);
};
